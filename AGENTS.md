...
5. **quant_pre_process symbolic shape inference fails on BOTH models**: the decoder (Expand broadcasting fixed dec_len=128 against symbolic enc_len in the causal-mask subgraph, which is a constant no-op here) and the encoder (Range with a symbolic num_samples-derived length). stage1b therefore runs with `skip_symbolic_shape=True` and `skip_optimization=True` (opt-in via `--enable-optimize` / `enable_optimize = true` in pipeline.toml): B = input graph structure unchanged + plain onnx C++ shape-inference value_info + pre-process metadata. Keeping the structure intact matters: graph rewrites (onnxsim, ORT_ENABLE_BASIC) shift which MatMuls `quantize_dynamic` quantizes ("const B") and measurably change int8 WER/CER — see reports/bench (A_sim path CER 0.276 vs raw-A path 0.206). With dynamic quantization the pre-processed B is only used for value_info; `quantize_static` (if ever pursued) would need a revisit.

"""Stage 4 benchmark: latency / memory / accuracy measurement.

Measures each variant with warmup + repeated greedy decode over eval wavs
(on-disk model, no runtime graph optimization). With --accuracy, also computes
WER/CER against reference transcripts.

    uv run python scripts/analysis/bench.py --model STT --measurements 30 --warmup 5
    uv run python scripts/analysis/bench.py --model STT --variants A,C,D_cpu
    uv run python scripts/analysis/bench.py --model STT --provider xnnpack
    uv run python scripts/analysis/bench.py --model STT --accuracy
"""
...
def main():
    ...
    ap.add_argument("--provider", default="cpu", help="EP to request (cpu, xnnpack, coreml)")
    ...
    banner(f"bench | host={hostname()} ort={ort.__version__} | variants={variants} provider={args.provider} measures={args.measurements}")
    ...
    for v in variants:
        ...
        print(f"  [{v:8}|{row['act_provider']:22}] p50={row['p50']:.1f} p95={row['p95']:.1f} "
